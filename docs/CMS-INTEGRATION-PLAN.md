> **Carried over from `parnian-cosmetics` and not yet re-written for VARAGH.**
> The strategy holds — the content layer is the same shape — but every literal
> in it is the engine's: the base path is `/varagh` here, the package is
> `varagh-studio`, and the content types are `Product` (a «کار»), `ArchiveItem`
> and the section interfaces in `src/types/content.ts`, not the cosmetics set.
> Read it for the approach, not for the field names.
