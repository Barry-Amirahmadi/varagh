> ## NOTE FOR THIS REPOSITORY — VARAGH, added 2026-09-21
>
> **This repo keeps the engine and replaces the design.** It was copied from
> `parnian-cosmetics`, whose brief this document is. Everything below about
> the Next.js static-export setup, the content architecture, the RTL rules,
> the accessibility floor, the contact-not-commerce architecture and the
> GitHub Pages traps still governs this repository unchanged. Everything below
> about *what the site looks like* does not.
>
> What was re-decided, and where:
>
> | Section | Ruling for VARAGH |
> |---|---|
> | §04 homepage structure | **Replaced.** Five bands: statement · register · three plates · disciplines · contact. No hero image, no brand-story band, no values band, no closing CTA band. Reusing the engine's sequence would have failed this build outright. |
> | §05 visual concept | **Replaced.** Brutalist-typographic. Flat, hard-ruled, one accent. |
> | §06 colour system | **Replaced.** Paper `#F2F2F0`, ink `#0A0A0A`, vermilion `#FF3B00`. The three depth planes are defined as the *same colour* on each side — flatness is the mechanism, see `src/app/tokens.css`. |
> | §07 typography | **Replaced.** Vazirmatn 900/400 for everything, JetBrains Mono (latin subset only) for catalogue marks. Markazi Text is gone. |
> | §10 product presentation | **Replaced.** A numbered register, not a spread of arrangements. `ProductRow` and its `arrangements` table are deleted. |
> | §11 motion system | **Replaced.** One reveal, 260ms, opacity and 8px. No ambient shade wash, no image settle, no hover zoom, no directional slide. `--shade`, `ShadeField` and `tone` are gone. |
> | §25 image art direction | **Replaced.** 24 square plates, overhead on concrete, hard directional light, one hot accent, no text in any frame. |
> | §42 inquiry architecture | **Narrowed.** Email and Instagram only. No WhatsApp, no phone — a studio is reached by writing to it. `src/lib/whatsapp.ts` is deleted and `SiteContent.contact` no longer has `phone`, `phoneHref` or `whatsapp`. |
> | §44.11 dark mode | **Unchanged — out of scope.** The dark ground here is a band, not a theme. |
> | §51 scope | **Unchanged.** A presented template, not a CMS-backed client site. |
>
> Structural-defect rulings carry over in full and are treated as binding:
> the sparse-auto-placement grid trap (`grid-template-areas` only, see the
> note in `src/app/components.css`), the unprefixed-image-path trap, the
> flattened RSC payloads, the `action="#"` form, and the base-path handling.
