 ...
 *
 *   node scripts/check-viewports.mjs /varagh/products/
 */
...
const path = args[0] ?? "/varagh/products/";
...
for (const width of VIEWPORTS) {
  const page = await browser.newPage({
    ...
    // Kept even though this site has no scroll motion: `--reduced` is how you
    // prove that, and a page whose layout changes under `prefers-reduced-
    // motion` has a transition doing structural work.
  ...
  // Walk the page so anything lazy has been asked for before measuring.
